...
def arima(training, horizon, n_coefs=100, best_model=False):
    """ARIMA model with detrending and FFT"""
    # Make sure we are just forecasting 2 days ahead (hardcoded below)
    assert horizon == (timedelta(days=2) - timedelta(minutes=15))
    assert len(training) >= 14 * 24 * 4  # 2 weeks of training data

    # 1) Remove linear trends
    days = 2
    day_len = 24*4
    training_len = len(training)
    t_and_f_len = np.array(range(0, training_len + days * day_len))
    p = np.polyfit(t_and_f_len[:training_len], training, 2)
    trend = p[0] * t_and_f_len**2 + p[1] * t_and_f_len #+ p[2]
    y_notrend = training - trend[:training_len]

    # 2) Remove some fourier components
    dft = DiscreteFourierTransform(n_coefs=n_coefs)
    X_dft = dft.fit_transform([y_notrend])
    real_idx = np.arange(1, n_coefs, 2)
    imag_idx = np.arange(2, n_coefs, 2)
    X_dft_new = np.c_[
        X_dft[:, :1],
       (X_dft[:, real_idx] +
        1j * np.c_[X_dft[:, imag_idx], np.zeros((1, ))])
    ]
    X_irfft = np.fft.irfft(X_dft_new, training_len)
    rest = y_notrend - X_irfft[0]

    # Prep exogeneous variables
    f_start = training.index[-1] + timedelta(minutes=15)
    f_end = f_start + horizon
    ix = pandas.date_range(
        start=f_start, end=f_end, freq='15T')
    exog_vars = np.column_stack(
        ([x.hour for x in training.index],
         [x.weekday() for x in training.index]))
    future_exog_vars = np.column_stack(
        ([x.hour for x in ix],
         [x.weekday() for x in ix]))

    # Run an ARIMA model on the rest
    if best_model:
        model = auto_arima(
            rest,
            exogenous=exog_vars,
        ...
        model.fit(rest, exogenous=exog_vars)
        yhat = model.predict(n_periods=days*day_len,
                             exogenous=future_exog_vars)
    else:
    ...
    r = pandas.DataFrame(
        index=ix,
        data={'yhat': (yhat +
                       X_irfft[0][one_week:one_week+days*day_len] +
                       trend[training_len:])})
